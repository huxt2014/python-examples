...
query(User).join(Address)

# inner join with condition
query(User).join(User.addresses)               # through relationship.
query(User).join('addresses')                  # the same
query(User).join(Address, User.addresses)         # the same, specify constrain
query(User).join(Address, User.id==Address.user_id)  # any condition
...
# the select Query instance will not emit a query until the following method
# invoked
...
# batch update and delete will emit the query immediately, return the number of
# row that changed
changed_number = (session.query(User)
                         .filter_by(name='a')
                         .update({user.name: 'b'}, synchronize_session=False))

delete_number = session.query(User).filter_by(ID=6).delete()
...
# construct a insert query, and limit columns using values. The actual data is
# not rendered into the sql, until the statement is executed.
ins = user.insert().values(name='name', email='email')
con.execute(ins)

# The insert statement is compiled at execution time based on the parameters.
ins = user.insert()
con.execute(ins, name='', email='')

# To issue many inserts using DBAPI’s executemany() method. Each dictionary 
# must have the same set of keys, because the Insert statement is compiled
# against the first dictionary in the list.
...
select([user])               # table object will be expanded to columns
select([user.c.name, user.c.email])            # select several columns
select([user.c.id.label('user_id')])           # label
select([user, address])      # join implicitly, get all columns.
...
# result #########################################################
# The result, known as a ResultProxy object, is analogous to the DBAPI cursor
# object.
