...
# basic query #################################################
#     The class Query is used for ORM-level SQL construction. 
#And then return the query instance.
...
session.query(User, Address)                  # m*n rows
session.query(User).join(Address)             # m rows


# identity set ################################################
#     All the instance corresponding to the same row in the database are
# identical in the identity map.
...
## emit a query and load relationship objects during the first qualification if
## no eager load is applied. 
User.addresses
...
session.query(User).options(joinedload('addresses')).join(User.addresses)             # 
                                           # 
## delete through relationship using secondary argument
user.addresses.remove(oneaddress)  # delete from secondary table automatically
session.delete(oneaddress)        # whether delete from secondary table depends on the configuration
